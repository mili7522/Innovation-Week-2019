...
### Load data
def load_and_train(folder):
    x = np.load(os.path.join(data_folder, folder + 'x_{}.npy'.format(fill_type)), mmap_mode = 'r')
...
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        pd.DataFrame(val_kappas).to_csv(os.path.join(model_path, 'history', model_name) + '-kappa.csv', header = False, index = False)
        return
